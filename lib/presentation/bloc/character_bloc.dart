...
class CharacterBloc extends Bloc<CharacterEvent, CharacterState> with HydratedMixin { // or => class CharacterBloc extends HydratedBloc<CharacterEvent, CharacterState>
    ...
    on<CharacterEventFetch>((event, emit) async {
      ...
      try {
        Character _characterLoaded = await characterRepo
            ...
            .timeout(const Duration(seconds: 5)); //чтобы не подвисал блок 
        emit(CharacterState.loaded(characterLoaded: _characterLoaded));
      } catch (_) {
        emit(const CharacterState.error());
        rethrow; //пускает ошибку дальше
      }
    });
  }

  //чтобы это работало надо в CharacterState реализовать factory CharacterState.fromJson(Map<String, dynamic> json) => _$CharacterStateFromJson(json);
  @override //востановление состояния
  CharacterState? fromJson(Map<String, dynamic> json) => CharacterState.fromJson(json);

  @override // сохранение состояния
